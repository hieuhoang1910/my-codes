...
% amplitude values for different modes (updated based on provided data)
...
% Adding contour for Amplitude Mode 1
contourf(X, Y, Z_amp_mode1, 50, 'LineStyle', 'none'); % 50 contour levels for better visualization
colorbar;
...
xlabel('Span (X) [in]');
ylabel('Chord (Y) [in]');
title('Amplitude values at each nodes - Mode 1');
grid on;
...
contourf(X, Y, Z_amp_mode2, 50, 'LineStyle', 'none'); % 50 contour levels for better visualization
colorbar;
...
xlabel('Span (X) [in]');
ylabel('Chord (Y) [in]');
title('Amplitude values at each nodes - Mode 2');
grid on;
legend('show', 'Location', 'bestoutside');
axis equal;
hold off;
